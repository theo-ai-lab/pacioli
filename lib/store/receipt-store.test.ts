...
describe("receipt store", () => {
  it("memory: saves, gets, lists, and aggregates stats", () => {
    const s = createMemoryStore();
    s.save(mk("r1", true, []));
    s.save(mk("r2", false, ["OVERSPEND", "SCOPE_CREEP"]));
    s.save(mk("r3", false, ["OVERSPEND"]));
  ...
  it("memory: a replay is an EVENT, not a new receipt (content-addressed dedupe + seenCount)", () => {
    const s = createMemoryStore();
    s.save(mk("r1", false, ["OVERSPEND"]));
    s.save(mk("r1", false, ["OVERSPEND"]));
  ...
  it("node:sqlite: persists durably and counts replays, keeping the FIRST createdAt", async () => {
    ...
    s.save(mk("x1", false, ["OVERSPEND"], 111));
    s.save(mk("x1", false, ["OVERSPEND"], 999)); // replay later — audit log keeps first-seen time
    s.save(mk("x2", true, []));
  ...
  it("memory: bounded at MEMORY_CAP — a flood evicts oldest instead of growing without limit", () => {
    const s = createMemoryStore();
    for (let i = 0; i < 10_001; i++) s.save(mk(`f${i}`, true, [], i));
